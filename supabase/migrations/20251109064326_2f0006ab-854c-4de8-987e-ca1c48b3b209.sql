-- ============================================================
-- Deliverable A: Campaign KPIs View
-- ============================================================
-- Purpose: Aggregated campaign-level metrics for dashboards
-- Security: Relies on underlying RLS of base tables (tenant-scoped)
-- ============================================================

CREATE OR REPLACE VIEW public.vw_awareness_campaign_kpis AS
SELECT 
  c.tenant_id,
  c.id AS campaign_id,
  ...
  -- Active days: days since start_date (date subtraction returns integer)
...
-- Optional LEFT JOIN to quiz_submissions for score prioritization
...
-- Grant SELECT to authenticated users (RLS on base tables will filter)
GRANT SELECT ON public.vw_awareness_campaign_kpis TO authenticated;

COMMENT ON VIEW public.vw_awareness_campaign_kpis IS 
'Campaign-level KPIs aggregating participants, scores, and completion metrics. Tenant-scoped via base table RLS.';


-- ============================================================
-- Deliverable B: Daily Engagement View
-- ============================================================
-- Purpose: Track daily participant activity (started, completed, scores)
-- Security: Tenant-scoped via underlying RLS
-- Note: Returns sparse days (only days with activity)
-- ============================================================

CREATE OR REPLACE VIEW public.vw_awareness_daily_engagement AS
WITH daily_completed AS (
  -- Daily completions from campaign_participants.completed_at
  SELECT 
    p.tenant_id,
    p.campaign_id,
...
daily_started AS (
  -- Daily starts from module_progress.started_at (first module started)
  SELECT 
    mp.tenant_id,
    mp.campaign_id,
...
'Daily engagement metrics: started_delta, completed_delta, avg_score per day. Sparse (only active days). Tenant-scoped via base RLS.';