...
export default function ThirdPartyVendors() {
  ...
  const filteredVendors = vendors?.filter((vendor) => {
    ...
    if (filter === 'high-risk') return vendor.overall_risk_level === 'high' || vendor.overall_risk_level === 'critical';
    return true;
  });
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-destructive">
                {vendors?.filter((v) => v.overall_risk_level === 'high' || v.overall_risk_level === 'critical')
                  .length || 0}
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <h3 className="font-semibold text-lg">{vendor.vendor_name_ar}</h3>
                  {vendor.website && (
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {vendor.overall_risk_level && (
                  <Badge variant={getRiskBadgeVariant(vendor.overall_risk_level)}>
                    {vendor.overall_risk_level === 'critical' ? 'حرجة' :
                     vendor.overall_risk_level === 'high' ? 'عالية' :
                     vendor.overall_risk_level === 'medium' ? 'متوسطة' :
                     vendor.overall_risk_level === 'low' ? 'منخفضة' : 'غير محددة'}
                  </Badge>
                )}
              </div>
